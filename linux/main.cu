...
void gpuWorker(int gpuId, Int rangeOffset, Int chunkSize) {
    cudaSetDevice(gpuId);
    ...
    for (int i = 0; i < outputSize; i++) buffDeviceResult[i] = false;

    cudaMalloc((void**)&dev_buffDeviceResult, outputSize * sizeof(bool));
    cudaMemcpy(dev_buffDeviceResult, buffDeviceResult, outputSize * sizeof(bool), cudaMemcpyHostToDevice);
    delete[] buffDeviceResult;

    uint64_t* buffResult = new uint64_t[COLLECTOR_SIZE];
    uint64_t* dev_buffResult;
    cudaMalloc((void**)&dev_buffResult, COLLECTOR_SIZE * sizeof(uint64_t));
    cudaMemcpy(dev_buffResult, buffResult, COLLECTOR_SIZE * sizeof(uint64_t), cudaMemcpyHostToDevice);

    bool* buffCollectorWork = new bool[1];
    buffCollectorWork[0] = false;
    bool* dev_buffCollectorWork;
    cudaMalloc((void**)&dev_buffCollectorWork, 1 * sizeof(bool));
    cudaMemcpy(dev_buffCollectorWork, buffCollectorWork, 1 * sizeof(bool), cudaMemcpyHostToDevice);

    cudaMalloc((void**)&dev_buffRangeStart, NB64BLOCK * sizeof(uint64_t));

    // Offset the RANGE_START for this GPU
    Int gpuRangeStart = RANGE_START + rangeOffset;
    __Load(buffRangeStart, gpuRangeStart.bits64);
    cudaMemcpy(dev_buffRangeStart, buffRangeStart, NB64BLOCK * sizeof(uint64_t), cudaMemcpyHostToDevice);

    const uint32_t expectedChecksum = IS_CHECKSUM ? CHECKSUM.GetInt32() : 0;

    dim3 threads(BLOCK_THREADS);
    dim3 blocks(BLOCK_NUMBER);

    if (COMPRESSED) {
        kernelCompressed<<<blocks, threads>>>(dev_buffDeviceResult, dev_buffCollectorWork, dev_buffRangeStart, THREAD_STEPS, expectedChecksum);
    } else {
        kernelUncompressed<<<blocks, threads>>>(dev_buffDeviceResult, dev_buffCollectorWork, dev_buffRangeStart, THREAD_STEPS, expectedChecksum);
    }
...
void launchOnAllGPUs() {
    ...
    for (int i = 0; i < GPU_COUNT; ++i) {
        gpuThreads.emplace_back([=]() {
            cudaSetDevice(i);
            printf("Launching on GPU %d\n", i);
            // TODO: Add kernel launch with keyspace offset based on i
            // Example: runKernelWithOffset(i * chunkSize, chunkSize);
        });
    }
